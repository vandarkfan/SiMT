...
model = SiTM(num_ent = KG.num_ent, num_rel = KG.num_rel, ent_vis = KG.ent_vis_matrix, rel_vis = KG.rel_vis_matrix, \
...
# optimizer = torch.optim.Adagrad(model.parameters(), lr = args.lr, weight_decay = args.decay)
...
for epoch in range(last_epoch + 1, args.num_epoch + 1):
    total_loss = 0.0
    for batch, label1, label2 in KG_Loader:


        ent_embs, rel_embs = model()

        rhs_scores, lhs_scores = model.score(ent_embs, rel_embs, batch.cuda())
        loss = loss_fn(rhs_scores, label1.cuda()) #+ loss_fn(lhs_scores, label2.cuda())
    ...
    if (epoch) % args.valid_epoch == 0:
        ...
        with torch.no_grad():
            ...
            for triplet in tqdm(KG.valid):
                ...
                rhs_scores, lhs_scores = model.score(ent_embs, rel_embs, torch.tensor([[h,r,t]]).cuda())
                # head_rank = calculate_rank(lhs_scores[0].cpu().numpy(), h, KG.filter_dict[(-1, r, t)])
                # tail_score = model.score(ent_embs, rel_embs, torch.tensor([[h,r,t]]).cuda())[0].detach().cpu().numpy()
